...
class VeggieBoil(Kitchen):
    ...
    def _setup_kitchen_references(self):
        ...
        self.counter_sink = self.register_fixture_ref("counter_sink", dict(id=FixtureType.COUNTER, ref=self.sink, size=(0.5, 0.5)))
        self.stove = self.register_fixture_ref("stove", dict(id=FixtureType.STOVE))
        self.counter_stove = self.register_fixture_ref("counter_stove", dict(id=FixtureType.COUNTER, ref=self.stove))
        self.init_robot_base_pos = self.sink

    def get_ep_meta(self):
        ep_meta = super().get_ep_meta()
        food_name = self.get_obj_lang("food")
        ep_meta["lang"] = f"""Pick up the pot and place it in the sink. Then turn on the sink and let the pot fill up with water. Then turn the sink off and move the pot to the stove.
