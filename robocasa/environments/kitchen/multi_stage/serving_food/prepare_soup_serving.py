...
class PrepareSoupServing(Kitchen):
    ...
    def _setup_kitchen_references(self):
        ...
        self.cabinet = self.register_fixture_ref("cab", dict(id=self.cab_id, ref=self.stove))
        self.counter = self.register_fixture_ref("counter", dict(id=FixtureType.COUNTER, ref=self.stove))
        self.init_robot_base_pos = self.cabinet

    def get_ep_meta(self):
        ep_meta = super().get_ep_meta()
        ep_meta["lang"] = f"Open the cabinet and move the ladle to the pot. Then close the cabinet."
        return ep_meta
