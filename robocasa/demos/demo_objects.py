...
if __name__ == "__main__":
    ...
    if "aigen" in obj_registries:
        aigen_objs_path = os.path.join(robocasa.__path__[0], "models/assets/objects/aigen_objs")
        if os.path.exists(aigen_objs_path) is False:
            print("Unable to find AI-generated objects locally. Downloading dataset files.")
            download_and_extract_zip(
                url="https://utexas.box.com/shared/static/os3hrui06lasnuvwqpmwn0wcrduh6jg3.zip",
                folder=aigen_objs_path,
                check_folder_exists=False,
        )
        print(colored(f"Ending script. Rerun script to use new AI-generated objects", "yellow"))
    ...
    while True:
        if args.mjcf is not None:
            ...
            print()
            print(colored(f"Category: {cat}", "yellow"))
            print(colored(f"AI-Generated? {aigen}", "yellow"))
            print(colored(f"Model path: {filepath}", "yellow"))
        ...
        if args.mjcf is None:
            print()
            print("(Press N to view another object, Q to end demo)")
            print()
        else:
            print("(Press Q to end demo)")
            print()
